...
- [ ] Position key uses `sender`, which is the *router* under v4, so positions from the same
      router collide. Track the real owner (or require a unique salt) before the JIT defense
      is trustworthy.
...
- [ ] `JITLiquidityAttacker` — add before, remove after, fee capture measured
- [ ] `OracleManipulator` — single-block price push to an extreme
- [ ] `DonationGriefer` — donation-based accounting abuse
- [ ] Each run twice: vanilla vs Aegis
- [ ] Benchmark table emitted as markdown, committed, regenerated by CI
