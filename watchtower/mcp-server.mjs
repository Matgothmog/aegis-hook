...
server.tool(
  ...
  async ({ poolId, chain, hook }) => {
    ...
    const cfgRaw = await ethCall(url, hook, "0x037aadbe" + id);
    ...
    const cfg = {
      ...
      mevTaxFloorGwei: Number(unsignedWord(w[3])),
      maxTickDeviation: Number(unsignedWord(w[4])),
      cooldownBlocks: Number(unsignedWord(w[5])),
      minPositionAgeBlocks: Number(unsignedWord(w[6])),
      configured: Number(unsignedWord(w[8])) === 1,
    };
    ...
    return text(
      `Pool ${poolId} on ${chain}\n` +
        ...
        `  MEV tax floor         ${cfg.mevTaxFloorGwei} gwei${cfg.mevTaxFloorGwei === 0 ? " (no ambient tip on this chain)" : " of ambient tip, untaxed"}\n` +
